...
export default function FounderActivate() {
  ...
  const submit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (code === "PURCHASE_EMAIL_MISMATCH") setError("Le reçu doit appartenir à la même adresse email que votre compte Cele One.");
      else if (code === "PAYMENT_ALREADY_LINKED") setError("Ce reçu est déjà associé à un autre compte.");
      else if (code === "FOUNDER_REFERENCE_NOT_FOUND") setError("Ce Founder ID est introuvable. Revenez d'abord le générer sur la page Founder's Pass.");
      else if (code === "FOUNDER_REFERENCE_ALREADY_USED") setError("Ce Founder ID est déjà lié à un autre paiement.");
      else if (code === "FOUNDER_ID_MISMATCH") setError("Le Founder ID saisi ne correspond pas à celui enregistré pendant la finalisation du paiement.");
      else if (code === "FOUNDER_ID_NOT_FOUND_IN_PAYMENT") setError("Aucun Founder ID n'a été retrouvé dans ce paiement. Collez votre Founder ID si vous ne l'avez pas ajouté pendant la finalisation.");
      else if (code === "INVALID_CLIENT" || code === "INVALID_ORIGIN") setError("La requête d'activation a été refusée. Rechargez la page puis réessayez.");
      else if (code === "ACTIVATION_VERIFICATION_INCOMPLETE") setError("La verification du paiement est incomplète. Réessayez dans quelques instants.");
      else if (code === "FIREBASE_SERVICE_ACCOUNT_NOT_CONFIGURED") setError("L'activation finale et la génération du certificat ne sont pas encore configurées sur le backend Firebase.");
      else if (code === "FIREBASE_TOKEN_ERROR" || code.startsWith("FIRESTORE_") || code.startsWith("STORAGE_")) setError("La validation du paiement est réussie, mais la création du pass ou du certificat a échoué côté serveur.");
      else if (code === "SALE_NOT_COMPLETED" || code === "PAYMENT_NOT_SUCCESSFUL") setError("Ce reçu n'est pas encore confirmé comme paiement réussi.");
      else if (code === "PRODUCT_MISMATCH") setError("Ce reçu ne correspond pas au Founder's Pass officiel.");
    ...
    <div className="mx-auto max-w-5xl space-y-6 py-6">
      ...
      >
        <div className="relative max-w-3xl">
          ...
          Collez votre id d'achat, votre reçu ou une capture de la finalisation du paiement. Si vous avez bien ajouté votre Founder ID pendant le paiement, nous le récupérons automatiquement.
          </p>
        </div>
      </section>
      ...
      {success ? (
        <div className="rounded-[24px] border border-emerald-200 bg-emerald-50 p-6 shadow-[0_10px_30px_rgba(8,24,40,0.05)]">
          <div className="text-lg font-bold text-emerald-900">Votre demande d&apos;activation a été soumise.</div>
          <p className="mt-2 text-sm font-medium leading-7 text-emerald-800">Le paiement a été relié à votre Founder ID et votre pass passe maintenant en revue d&apos;activation.</p>
        ...
        <form onSubmit={submit} className="rounded-[24px] border border-slate-200 bg-white p-8 shadow-[0_10px_30px_rgba(8,24,40,0.05)]">
          <div className="mb-6">
            <div className="text-sm font-bold uppercase tracking-[0.2em] text-[#0f8c68]">Détails de verification</div>
            ...
            <p className="mt-2 text-[15px] font-medium leading-8 text-slate-600">
              Utilisez votre Founder ID si nécessaire, puis ajoutez votre id d&apos;achat, votre reçu ou votre capture de paiement.
            </p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            />
            <Field
              label="Reçu, id d'achat ou capture de paiement"
              value={form.receiptReference}
              onChange={(v) => setField("receiptReference", v)}
              placeholder="Ex: reçu Chariow ou capture de finalisation"
              required
            />
          </div>

          <div className="mt-5 rounded-[22px] bg-[#f8fbfd] p-5 text-sm font-medium leading-7 text-slate-600">
            Le reçu doit correspondre au paiement réussi du Founder's Pass officiel et utiliser la même adresse email que votre compte Cele One. Le champ Founder ID reste disponible seulement si vous devez corriger ou compléter la verification.
          ...
          <button disabled={saving} className="mt-6 inline-flex min-h-[56px] items-center justify-center rounded-full bg-[#2ed06e] px-7 text-[15px] font-bold text-white shadow-[0_12px_28px_rgba(46,208,110,0.22)] hover:bg-[#28c464] disabled:opacity-60">
            {saving ? "Vérification..." : "Soumettre l'activation"}
          </button>
          <div className="mt-5 text-sm font-medium text-slate-500">
            <Link to="/founders" className="font-bold text-[#0f8c68] hover:underline">Retour à l'espace Founder&apos;s Pass</Link>
          </div>
        </form>
      )}
    </div>
