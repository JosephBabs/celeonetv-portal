...
import { auth } from "./firebase"; // Ensure this file exists in your project
...
const Navbar = ({ user, onLogout }) => (
  <nav className="navbar">
    ...
    <div className="nav-links">
      {user ? (
        <>
          <span>Hello, {user.email || user.username} ({user.role || 'User'})</span>
...
const LoginForm = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Mocking role assignment based on email for demonstration purposes
      // In a real app, you would fetch this from a database or custom claims
      const role = email.includes('admin') ? 'admin' : 'owner';

      // Pass Firebase credentials + role to parent
      onLogin({
        uid: firebaseUser.uid,
        email: firebaseUser.email,
        token,
        role,
        userId: role === 'admin' ? 'admin1' : 'owner1' // Mapping to mock IDs
      });
