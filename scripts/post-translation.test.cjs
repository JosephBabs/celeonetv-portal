...
(async () => {
  ...
  const calls=[];
  global.fetch=async (url,init)=>{const parsed=new URL(url);assert.equal(parsed.hostname,'translate.googleapis.com');const data={text:parsed.searchParams.get('q'),target:parsed.searchParams.get('tl'),source:parsed.searchParams.get('sl')};calls.push(data);assert.ok(init.signal);return Response.json([[[data.text.toUpperCase()]]]);};
  ...
  assert.equal(await translateText('line one\n\nline two','it',signal),'LINE ONE\n\nLINE TWO');
  global.fetch=async(url)=>url.startsWith('https:')?new Response('{}',{status:503}):Response.json({translatedText:'Fallback works'});
  assert.equal(await translateText('fallback probe','en',signal),'Fallback works');
  const aborted=new AbortController();aborted.abort();
  await assert.rejects(translateText('cancelled','fr',aborted.signal),{name:'AbortError'});
